    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                s_ch = (b2_y1 + b2_y2 - b1_y1 - b1_y2) * 0.5 + eps  # 计算预测边界框（b2）和实际边界框（b1）之间的空间宽度和高度差异。
                sigma = torch.pow(s_cw ** 2 + s_ch ** 2, 0.5)  # 计算围绕两个边界框的最小外接框的对角线长度
                sin_alpha_1 = torch.abs(s_cw) / sigma
                sin_alpha_2 = torch.abs(s_ch) / sigma  # 计算由宽度和高度差异形成的角度的正弦值
                threshold = pow(2, 0.5) / 2
                sin_alpha = torch.where(sin_alpha_1 > threshold, sin_alpha_2, sin_alpha_1)
                angle_cost = torch.cos(torch.arcsin(sin_alpha) * 2 - math.pi / 2)  # 计算角度的余弦值
                rho_x = (s_cw / cw) ** 2
                rho_y = (s_ch / ch) ** 2  # 计算空间宽度和高度的比率
                gamma = angle_cost - 2  # 通过角度成本减去2得到
                distance_cost = 2 - torch.exp(gamma * rho_x) - torch.exp(gamma * rho_y)  # 计算空间宽度和高度上的指数衰减成本
                omiga_w = torch.abs(w1 - w2) / torch.max(w1, w2)
                omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)  # 计算宽度和高度的相对差异
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)  # 计算形状成本，其中使用了指数衰减
                return iou - 0.5 * (distance_cost + shape_cost) + eps  # SIoU，最终返回 iou 减去空间成本、距离成本和形状成本的加权和
...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8):
    ...
    b1_x1, b1_y1, b1_x2, b1_y2 = pred.chunk(4, -1)  # 预测的边界框，格式为 (x_min, y_min, x_max, y_max)，形状为 (n, 4)。
    b2_x1, b2_y1, b2_x2, b2_y2 = target.chunk(4, -1)  # 相应的真实边界框，形状为 (n, 4)
    w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1 + eps  # 计算预测边界框的宽度和高度，eps：避免取对数时分母为零的小量
    w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + eps  # 计算真实边界框的宽度和高度

    b1_x_center, b1_y_center = b1_x1 + w1 / 2, b1_y1 + h1 / 2  # 计算预测边界框的中心坐标
    b2_x_center, b2_y_center = b2_x1 + w2 / 2, b2_y1 + h2 / 2  # 计算真实边界框的中心坐标
    center_distance = (b1_x_center - b2_x_center) ** 2 + (b1_y_center - b2_y_center) ** 2 + eps  # 计算中心点之间的距离
    wh_distance = ((w1 - w2) ** 2 + (h1 - h2) ** 2) / 4  # 计算宽度和高度之间的距离

    wasserstein_2 = center_distance + wh_distance  # 计算 Wasserstein 距离的平方，包括中心点距离和宽高距离
    return torch.exp(-torch.sqrt(wasserstein_2) / constant)  # 最终返回 Wasserstein Loss，其中使用了一个常数 constant 对 Wasserstein 距离进行缩放，并对其取指数
